...
public class BracketPane extends BorderPane {
        ...
        private EventHandler<MouseEvent> enter = mouseEvent -> {
                tmp1 = (BracketNode) mouseEvent.getSource();
                tmp1.setStyle("-fx-background-color: lightcyan;");
                
                tmp1.setEffect(new InnerShadow(10, Color.LIGHTCYAN));
                String text = "";
                        BracketNode n = (BracketNode) mouseEvent.getSource();
                        int treeNum = bracketMap.get(n);
        ...
        public boolean isFinalized() {
                return currentBracket.getFinalized();
        }
        ...
        private class Root extends Pane {
                ...
                public Root(int location) {
                        ...
                        createVertices(460, 200, 110, 20, 0, 0);
                        createVertices(350, 119, 110, 200, 1, 0);
                        createVertices(240, 60, 110, 100, 2, 200);
                        createVertices(130, 35, 110, 50, 4, 100);
                        createVertices(20, 25, 110, 25, 8, 50);
                        for (BracketNode n : nodes) {
                                n.setOnMouseClicked(clicked);
                                n.setOnMouseEntered(enter);
                                n.setOnMouseExited(exit);
                                // I used this to get a bracket guess, it also at some point got team guess -Svet
                                n.setGuess(currentBracket.getGuessType());
                            // Checks to make sure the bracket has been finalized and if it is create the bracket with red node or green nodes -Svet
                            if(!isFinalized()){
                                if (n.getGuess() == false){
        ...
        private class BracketNode extends Pane {
                ...
                private Label name;
                //Added a boolean guess to each bracketNode so it can be connected to team  -Svet
                ...
                // Another getter and setter for guesses for each node -Svet
        ...
        public void updateRed(){
            tmp1.setStyle("-fx-background-color: red;");
        }
}
